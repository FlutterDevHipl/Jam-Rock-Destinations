            ...
            children: [
              ListTile(
                ...
                onTap: () async {
                  ...
                  // Crop Image
                  final croppedPath =
                      await CustomWidget().cropImage(image.path);
  ...
  Future<void> driverRegistration(String token, final step) async {
    try {
      ...
      if (response['success'] == true) {
        ...
        print(
            "registration_token step 2 = ${response["data"]["registration_token"]}");
  ...
  Future<void> driverRegistration2(String token, int step) async {
    try {
      ...
      if (response["success"] == true) {
        ...
        Get.offAll(
            ProfileReviewView(token: response["data"]["registration_token"]));
        // Get.to(
        //   VehicleDetailsView(
        //     token: response["data"]["registration_token"],
        //     step: response["data"]["next_step"],
  ...
  Future<void> driverRegistration3(String token) async {
    try {
      ...
      final response = await ApiProvider()
          .postRequest1(apiUrl: AppConstants.register, data: fields);
      print(response);
